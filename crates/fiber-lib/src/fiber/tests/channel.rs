...
async fn test_ring_self_payments_then_restart_two_nodes() {
    ...
    // In restart stress runs retryable queues can be large, so keep a wide 120s timeout window.
    // If timeout is reached, continue to post-check assertions for full diagnostics.
    ...
    for (hash, _) in residual_by_hash.iter() {
